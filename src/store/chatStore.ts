...
export const useChatStore = create<ChatState>((set, get) => ({
  ...
  sendMessage: async (channelId, content, isAiResponse = false) => {
    try {
      ...
      if (inserted) {
        set((state) => ({
          messages: state.messages.some((m) => m.id === inserted.id)
            ? state.messages
            : [...state.messages, inserted as unknown as Message],
        }));
      }
    } catch (err) {
    ...
    channel
      .on(
        ...
        async (payload) => {
          ...
          if (data) {
            set((state) => ({
              messages: state.messages.some((m) => m.id === data.id)
                ? state.messages
                : [...state.messages, data as unknown as Message],
            }));
          }
        }
      )
